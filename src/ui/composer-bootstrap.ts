...
function start() {
  const post = (type: string, payload?: any) => {
    try {
      (window as any).CodexBridge?.post(type, payload);
    } catch {}
  };
  ...
  if (root) {
    const composer = initComposer(root, {
      placeholder: "Ask anythings (Ctrl+I)",
    ...
    if (sendBtn) {
      sendBtn.addEventListener("click", () =>
        send(composer.getValue(), composer.getAttachments?.())
      );
      // Initialize disabled state
      const initText = (composer.getValue() ?? "")
        .replace(/<[^>]*>/g, "")
        .trim();
    ...
    window.addEventListener("message", (ev) => {
      ...
      if (msg.type === "ui.insertText")
        composer.setValue(composer.getValue() + (msg.text ?? ""));
    });
